...
要启用审核并将审核事件打印到标准输出（即stdout），请为[`--auditDestination`](../../reference/program/mongod/#cmdoption-mongod-auditdestination)指定参数为'console'。例如：
...
例如，以下选项启用审核并将审核事件记录到相对路径'data/db/auditLog.json'的文件中：
...
与以BSON格式打印到文件相比，以JSON格式打印审核事件到文件的性能降低服务器性能。
...
例如，以下选项启用审核并将审核事件记录到相对路径'data/db/auditLog.bson'的文件中：
...
要查看文件的内容，请将文件传递给MongoDB实用程序 bsondump。例如，以下内容将审核日志转换为可读格式并输出到终端：
...
[配置审核过滤器](../configure-audit-filters/)，[审核](../../core/auditing/)，[系统事件审核消息](../../reference/audit-message/)